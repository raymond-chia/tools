...
- [x] `execute_skill` 產生技能 log 事件並 append（含名稱快照）
- [x] `process_reactions` 產生反應 log 事件並 append（含名稱快照）
- [x] `resolve_deaths` 產生死亡 log 事件並 append（已含於階段 2）
- [x] 新增 `ecs_logic/battle_log.rs`：`EffectEntry` → `LogEvent` 轉換（一筆 EffectEntry = 一筆 LogEvent），caster/reactor/trigger 名稱由呼叫端傳入快照，target 名稱查 World（查不到 fail fast）
