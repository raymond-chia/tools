...
- [x]（測試）死者從 `ReactionState.pending` 剔除（反應流程中打死目標 → pending 不含死者）
  - 補正（見文末「Code Review 補正 P1」）：亦須從 `decided` 剔除死者，否則 `process_reactions` 會 pop 出已死 reactor 而報錯。已加測試 `test_resolve_deaths_removes_dead_from_decided`
...
- [x] 新增 `resolve_deaths`：掃 HP≤0 → 產生死亡 log 事件（只記身分名稱快照）→ **批次移除**（全部從 TurnOrder 移除 → 全部 despawn → 只判一次開新一輪）→ 死當前單位則新當前單位跑 `begin_unit_turn` → 從 `ReactionState.pending` 剔除死者
  - 實作時額外加上：死者 despawn 後清除掛在死者身上（target == 死者）的孤兒 `AppliedBuff` entity（buff 是獨立 entity，不隨死者 entity 一併移除）。由 `remove_buffs_targeting` 處理，測試 `test_resolve_deaths_removes_dead_unit_buffs` 覆蓋
...
> **D10（新增決策）：log 產生由 UI 明確呼叫，不藏在 `execute_skill`／`process_reactions` 內部。**
> 原本（已撤回）的做法是在這兩個 core 函數**內部**自動 append log，但這讓函數多了難以察覺的副作用、且偏離「明確呼叫」原則。改為比照 D1 死亡處理：core 只提供 **`pub` 的 `append_skill_log`／`append_reaction_log`**，由 editor 在施放／反應之後**明確呼叫**來新增 log。
> 取捨：繼承「UI 可能漏呼叫 → 漏記 log」風險（與 D1 死亡處理同類，可接受；log 漏記比死亡漏處理更難察覺，但 editor 本就承擔編排責任）。

- [x]（測試）log 產生：技能/反應/死亡三種事件、名稱快照正確
  - `test_execute_skill_appends_skill_log`（技能）、`test_process_reactions_appends_reaction_log`（反應）、`test_resolve_deaths_appends_death_log`（死亡，含於階段 2）
- [x] 新增 `ecs_logic/battle_log.rs`：`EffectEntry` → `LogEvent` 轉換（一筆 EffectEntry = 一筆 LogEvent）。所有名稱快照（caster/reactor/target，皆從 entry 的 caster ID 或 CheckTarget 查 World）都在此查，呼叫端不需為 log 預先查任何 `OccupantTypeName`；反應特有的 trigger 不在 `EffectEntry` 內，由呼叫端以 `Occupant` 傳入身分、本函數查名。查不到 fail fast。
- [x] `append_skill_log`／`append_reaction_log` 為 `pub`，供 editor 呼叫；`execute_skill`／`process_reactions` 內部**不**自動 append（D10）。
- [x] `ProcessReactionResult::Executed` 加 `trigger: Occupant` 欄位（單次反應只對應單一 trigger），供 editor 取得 trigger 後呼叫 `append_reaction_log`。
- [x] `resolve_deaths` 產生死亡 log 事件並 append（已含於階段 2，死亡 log 仍在 `resolve_deaths` 內產生——死亡判定本就由 UI 編排呼叫 `resolve_deaths`，符合 D10 的「明確呼叫」精神）。

> **進度**：階段 1～3 完成且 `cargo test -p board` 全綠（70 passed）。階段 4 editor 改動完成且 `cargo check -p editor` 通過（手動戰鬥驗證待玩家執行）。階段 5 索引文檔已更新。

### 階段 4：editor 改讀 core log + 編排死亡（不寫測試，手動驗證）

> **注意**：階段 4 與 D10 高度相關——editor 移除 `battle_log.extend` 後，正是改呼叫 `append_skill_log`／`append_reaction_log`。反應處的 `Executed { effects }` 解構需同步改為 `Executed { effects, trigger }`（[battle.rs:1016](editor/src/tabs/level_tab/battle.rs:1016)），用 `trigger` 呼叫 `append_reaction_log`。editor 目前因 `Executed` 加了欄位、解構不全，**也會編譯不過**，需在階段 4 一併修正。

- [x] 移除 `LevelTabUIState.battle_log: Vec<EffectEntry>`（連同 `EffectEntry` import）
- [x] 移除三處 `battle_log.extend`（主動技能單目標、多目標確認、反應）
- [x] ~~移除 deployment 的 `battle_log.clear`~~（查證：deployment.rs 並無此呼叫，無須處理）
- [x] `execute_skill` 兩處呼叫後編排 `append_skill_log` + `resolve_deaths`
- [x] `process_reactions` 回傳 `Executed` 後解構 `trigger`，編排 `append_reaction_log` + `resolve_deaths`
- [x] 改寫 log 渲染為新 log 型別，並處理綁在舊 `EffectEntry` 的輔助函數：
  - `render_battle_log` 改吃 `&[LogEvent]`，改由 `render_form` 每幀以 `get_battle_log(&world)` 讀取
  - `render_effect_entry` → 改寫為 `render_log_event`（依 `LogEvent` 三種變體渲染，含死亡）
  - `format_check_target`／`format_check`／`format_effect` → 改寫為 `format_log_target`／`format_log_check`／`format_log_effect`（吃 Log 型別）
  - `find_unit_name_by_id`：已刪除（新 log 自帶名稱快照，不再事後反查）
- [x] 確認切換 `RightPanelView::Log` 的時機仍正確（三處施放/反應後仍設為 Log，行為不變）
- [ ] 編譯 + 手動戰鬥驗證（`cargo check -p editor` 已過；手動僅剩 editor UI 目視確認）
  - **邏輯已由 core 自動測試覆蓋**（施放 log、反應 log、死亡 log、死者移出 TurnOrder、剔除 pending／decided、移動者反應鏈死亡計畫作廢等，皆有對應 `test_*`；`cargo test -p board` 72 passed）。
  - 手動僅剩 **editor UI 渲染**（受 `rules/editor.md`「禁止替 editor crate 撰寫測試」約束、且為 egui 即時繪製難以自動化）：log 面板每幀正確渲染、死者從戰場格線消失、施放/反應後右側面板切到 Log view。
  - 補正（見文末「Code Review 補正 P2」）：「反應鏈打死移動者後 `force_advance_move` 的行為」已由 core 測試 `test_force_advance_move_after_mover_dies_in_reaction` 覆蓋，不再倚賴手動驗證。

### 階段 5：文檔

- [x] 更新 `rules/core-index.md`（新增 `resolve_deaths`、`get_battle_log`、`battle_log.rs` 模組與 `append_skill_log`／`append_reaction_log`、`domain/battle_log.rs`；移除 `remove_dead_unit`；修正 `end_current_turn`／`delay_current_unit` 回傳型別）
- [x] ~~更新 `rules/editor-index.md`~~（log 渲染函數皆為私有，依編輯規則不記錄；`render_form` 簽名未變，無須改動）
...
## Code Review 補正（實作後嚴格 review 發現，本計畫原未涵蓋）

實作完成、`cargo test -p board` 全綠後，對已勾選項目做嚴格 code review，發現兩個原計畫未涵蓋的缺陷。兩者都是 core 層能寫失敗測試的行為，依 TDD 先補紅燈測試、再修實作至轉綠。測試位於 `tests/ecs_logic/test_reaction.rs`。

### P1：`resolve_deaths` 只剔除 `pending`，未剔除 `decided`

- **問題**：D2 第 5 點與注意事項只提「從 `ReactionState.pending` 剔除死者」，遺漏 `decided` 佇列。editor 反應 loop 每次 `Executed` 後都呼叫 `resolve_deaths`；若 `decided` 中某個尚未執行的 reactor 在此時死亡（AOE／持續傷害等任意來源），`resolve_deaths` 不清 `decided`，下一次 `process_reactions` 會 pop 出死者並 `find_entity_by_occupant` 查不到已 despawn 的 entity → 回 `Err`。
- **原計畫為何漏**：靠「反應不會打死其他待反應者」的隱含假設掩蓋（現有技能反應目標恆為移動者/trigger，reactor 之間不互相攻擊），此假設未被攤開。
- **修正**：`resolve_deaths` 剔除死者時，`decided` 與 `pending` 一併 `retain`（[turn.rs](core/board/src/ecs_logic/turn.rs)）。
- **測試**：`test_resolve_deaths_removes_dead_from_decided`——兩 reactor 排入 `decided`，執行第一個後打死第二個並 `resolve_deaths`，斷言第二次 `process_reactions` 正常回 `Done`。

### P2：反應打死移動者後，`force_advance_move` 誤移遞補的當前單位

- **問題**：editor 反應 loop 的 `Done` 分支無條件呼叫 `force_advance_move`。若反應鏈把**移動者本身**打死，`resolve_deaths` 已將其 despawn、移出 TurnOrder，當前單位遞補為他人；`force_advance_move` 卻以 `get_current_unit` 取得遞補者、套用死者遺留的 `MovementPlan`，把存活的遞補單位硬拖到死者路徑的下一格。
- **原計畫為何漏**：僅有一句泛泛的「打死單位手動戰鬥驗證待玩家執行」，未點出「移動者自己在反應鏈中死亡」這條路徑，且該行為本該由 core 測試覆蓋而非推給手動驗證。
- **修正語意**：移動者死亡 → 其移動計畫作廢。
  - `MovementPlan` 新增 `occupant` 欄位記錄計畫擁有者（[resources.rs](core/board/src/ecs_types/resources.rs)），`plan_move` 寫入。
  - `force_advance_move` 比對「計畫擁有者 ≠ 當前單位」→ 移除 `MovementPlan`、不動任何單位、回走 0 步的 `Completed`（[movement.rs](core/board/src/ecs_logic/movement.rs)）。選「作廢 + 靜默」而非「報錯」，是因 editor `Done` 分支無條件呼叫，報錯會讓正常的移動者死亡流程噴錯。
- **取捨**：判定用「計畫擁有者 ≠ 當前單位」而非「查移動者是否存活」，貼合 `force_advance_move` 推進**當前**單位的契約。副作用——若未來出現「移動中斷後當前行動權交給他人、但原移動者仍存活」的情境，此計畫也會被作廢；目前架構下反應不改變當前行動單位，不會發生，日後若改變需重新檢視此判定。
- **測試**：`test_force_advance_move_after_mover_dies_in_reaction`——移動者壓低 HP 被反應打死後，斷言 `force_advance_move` 不移動存活的遞補單位（留原位）。
