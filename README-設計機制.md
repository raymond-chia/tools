...
- 格子是正方形
  ...
  - 格子沒有高度
  - 格子有明亮、昏暗、黑暗
    - 昏暗會降低命中與閃避。黑暗會降低命中與閃避。如果能感知到攻擊者/目標，可以無視對應光線懲罰
    - 不允許單體攻擊感知不到的地方。所以如果沒有聽覺定位技能，只能知道敵方單位在哪裡，但是不能單體攻擊黑暗的格子
    - 聽覺可以想像成視線系統，只檢查直線距離 & 直線是否有地圖物件會阻擋 (bresenham line)
    - 正常單位無法靠聽覺定位別人，只有某些敵人可以
    - 聲音就只是第二套視覺系統，但是被不同的地圖物件阻擋。你要改叫它回聲定位也可以
