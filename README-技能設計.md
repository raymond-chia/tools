...
- 移動過程中觸發的 reactions（如借機攻擊）需要立即執行並影響後續移動
- 每移動一格就檢查並讓玩家決定如何處理 reaction
- 沒有 reaction 時批次移動（性能優化），有 reaction 時逐步移動（讓玩家控制）

**UI 層智能處理流程**：

1. **預先檢查**：掃描整條路徑是否有任何 reaction
2. **沒有 reaction**：
   - 批次執行所有移動
   - 一次更新 UI 顯示最終位置
   - ✅ 流暢，沒有多餘暫停
3. **有 reaction**：
   - 逐步執行每一格移動
   - 觸發 reaction 時立即更新 UI 到當前位置
   - 彈出對話框讓玩家選擇（執行哪個技能或跳過）
   - 執行完 reaction 後檢查單位是否還能繼續
   - ✅ 玩家完全掌控每一步
